...
Make sure to have cut up a 3ds exefs codebin (or 3dsxdump a homebrew program) into .text (code.bin), .rodata (rodata.bin) and .data (data.bin) and save the files somewhere.  
Don't need to add or remove zero padding to page align, it's done on load.  

Needs MINGW64 environment of MSYS2 on Windows, this will not compile under MSVC  
Maybe don't use this ? Or read the helper scripts and `recompiler.cpp` for examples.  
Not a great tool, not great code, only barely works, written in ~~a week's worth of~~ way too many late nights.  
