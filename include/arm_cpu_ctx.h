...
typedef union arm_code_bank_entry {
    struct {
        void* entry_arm;
        void* entries_thumb[2];
    };
    void* entries[3];
} arm_code_bank_entry;
typedef struct arm_code_bank {
    u32_t start_addr;
    u32_t end_addr;
    struct arm_code_bank* next_bank;
    arm_code_bank_entry labels[0]; // actually (end_addr - start_addr) / 4 entries
} arm_code_bank;
